...
func (c *OVNNbClient) getLogicalSwitchQos(lsName, externalPort, v4Eip, direction, routerCrPort string) ([]*ovnnb.QoS, error) {
	fileterFunc := func(qos *ovnnb.QoS) bool {
		...
		if qos.Match != "" {
			...
			// Check if both gatewayPort and v4Eip exist in Match string， and using string contains method
...
func (c *OVNNbClient) listLogicalSwitchQosByFilter(lsName string, filter func(qos *ovnnb.QoS) bool) ([]*ovnnb.QoS, error) {
	...
	ovnQoSMap, err := c.batchGetQosByUUIDs(ls.QOSRules)
	if err != nil {
		klog.Error("batch get qos by uuids failed, %v", err)
		return nil, err
	}
	for _, qos := range ovnQoSMap {
...
// batchGetQosByUUIDs get qos by uuids
func (c *OVNNbClient) batchGetQosByUUIDs(uuids []string) (map[string]*ovnnb.QoS, error) {
	var qosList []*ovnnb.QoS
	cond := c.WhereCache(func(qos *ovnnb.QoS) bool {
		return slices.Contains(uuids, qos.UUID)
	})
	if err := cond.List(context.Background(), &qosList); err != nil {
		return nil, err
	}

	qosMap := make(map[string]*ovnnb.QoS, len(qosList))
	for _, qos := range qosList {
		qosMap[qos.UUID] = qos
	}
	return qosMap, nil
